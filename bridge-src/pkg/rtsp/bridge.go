...
func (wb *WebRTCBridge) setupPeerConnection(webRTCConfig *tuya.WebRTCConfig) error {
	...
	// LAN-only hardening (patched): never let media traverse Tuya's internet TURN relay.
	{
		hostOnly := os.Getenv("TUYA_IPC_HOST_ONLY") == "1"
		var lanICE []pion.ICEServer
		for _, srv := range iceServers {
			...
			for _, u := range srv.URLs {
				...
				if hostOnly && strings.HasPrefix(lu, "stun:") {
					continue
				}
				urls = append(urls, u)
			}
			if len(urls) > 0 {
				srv.URLs = urls
				lanICE = append(lanICE, srv)
			}
		}
		iceServers = lanICE
		core.Logger.Info().Msgf("LAN-only: %d ICE server(s) after stripping relay (hostOnly=%v)", len(iceServers), hostOnly)
	}
...
func (wb *WebRTCBridge) setupMQTTCameraClient(webRTCConfig *tuya.WebRTCConfig) {
	...
	wb.cameraClient.HandleCandidate = func(candidate tuya.CandidateFrame) {
		...
		if lc := strings.ToLower(candidateStr); strings.Contains(lc, "typ relay") || (os.Getenv("TUYA_IPC_HOST_ONLY") == "1" && strings.Contains(lc, "typ srflx")) {
			core.Logger.Info().Msgf("LAN-only: dropping remote candidate: %s", candidateStr)
			return
		}
...
func (wb *WebRTCBridge) createAndSendOffer() error {
	wb.peerConnection.OnICECandidate(func(candidate *pion.ICECandidate) {
		if candidate != nil {
			...
			if gc := strings.ToLower(candidate.ToJSON().Candidate); strings.Contains(gc, "typ relay") || (os.Getenv("TUYA_IPC_HOST_ONLY") == "1" && strings.Contains(gc, "typ srflx")) {
				core.Logger.Info().Msgf("LAN-only: not sending local candidate: %s", candidate.ToJSON().Candidate)
				return
			}
