...
func promptPassword() (string, error) {
	if p := os.Getenv("TUYA_PASSWORD"); p != "" {
		return p, nil
	}
...
func performPasswordAuthentication(region tuya.Region, email string) (*tuya.SessionData, error) {
	...
	countryCode := os.Getenv("TUYA_COUNTRY_CODE")
	if countryCode == "" {
...
func performQRAuthentication(region tuya.Region, email string) (*tuya.SessionData, error) {
	...
	fmt.Println("QRCONTENT::tuyaSmart--qrLogin?token=" + qrCodeToken)
	...
	email = loginResult.Email // patched: adopt scanned account
